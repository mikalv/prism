//! Graph backend for relationship storage and traversal.
//!
//! Provides in-memory graph storage with optional persistence via SegmentStorage.
...
/// Graph backend with SegmentStorage support.
pub struct GraphBackend {
    /// Collection name
    collection: String,
    /// Shard identifier (default: "default")
    shard: String,
...
impl GraphBackend {
    /// Create a new in-memory graph backend.
    pub fn new(collection: impl Into<String>, config: &GraphBackendConfig) -> Self {
        Self {
            collection: collection.into(),
            shard: "default".to_string(),
            nodes: Arc::new(RwLock::new(HashMap::new())),
            edges: Arc::new(RwLock::new(HashMap::new())),
            edge_configs: config.edges.clone(),
            storage: None,
        }
    }

    /// Create a graph backend with SegmentStorage persistence.
    ...
    /// Create a graph backend with custom shard.
    pub fn with_shard(mut self, shard: impl Into<String>) -> Self {
        self.shard = shard.into();
        self
    }

    /// Initialize the backend, loading persisted data if available.
            ...
            if storage.exists(&nodes_path).await.unwrap_or(false) {
                match self.load_from_storage(storage.as_ref()).await {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            error = %e,
                            "Failed to load graph from storage, starting fresh"
                        );
                    }
                }
            }
    ...
    ///
    /// Returns all node IDs reachable within `max_depth` hops following
    ...
    ///
    /// Returns the path as a list of node IDs (including start and target),
    ...
    fn nodes_path(&self) -> StoragePath {
        StoragePath::graph(&self.collection, &self.shard, "nodes.json")
    }

    fn edges_path(&self) -> StoragePath {
        StoragePath::graph(&self.collection, &self.shard, "edges.json")
    }
    ...
    async fn persist(&self) -> Result<()> {
        ...
        let (nodes_data, edges_data) = {
            ...
            // Serialize nodes and edges separately for more efficient updates
...
mod tests {
    ...
    fn test_config() -> GraphBackendConfig {
        GraphBackendConfig {
            path: String::new(),
            edges: vec![
                EdgeTypeConfig {
    ...
    async fn test_add_and_get_node() {
        ...
        backend.add_node(node.clone()).await.unwrap();

        let retrieved = backend.get_node("n1").unwrap();
    ...
    async fn test_bfs() {
        ...
        let result = backend.bfs("a", "related", 2);
        assert!(result.contains(&"b".to_string()));
        assert!(result.contains(&"c".to_string()));
        assert!(!result.contains(&"d".to_string())); // depth 3
    }
    ...
    async fn test_persistence_with_segment_storage() {
        ...
        // Create new backend and verify data loaded
        {
            ...
            let edges = backend.get_edges("n1");
            assert_eq!(edges.len(), 1);
