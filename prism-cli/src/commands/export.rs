...
pub async fn run_export(
    ...
    match format {
        ExportFormat::Portable => {
            export_portable_collection(data_dir, schemas_dir, collection, &output_path, progress.as_ref())
                .await?;
        }
...
async fn export_portable_collection(
    ...
    let manager = prism::collection::CollectionManager::new(schemas_dir, text_backend, vector_backend)
